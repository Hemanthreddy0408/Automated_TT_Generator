...
public class TimetableGenerationService {
    ...
    for (int attempt = 1; attempt <= MAX_ATTEMPTS; attempt++) {
        try {
            ...
            List<TimetableEntry> result =
                    generateForSection(sectionId, commit, null, null);

            if (commit) {
                auditLogService.logAction(
                        "TIMETABLE",
                        "GENERATE",
                        "Generated timetable for Section ID: " + sectionId,
                        "System/Admin"
                );
     ...
     */
    private List<TimetableEntry> generateForSection(
            String sectionId, boolean commit,
